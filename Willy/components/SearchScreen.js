...
export default class SearchScreen extends Component {
  ...
  componentDidMount = async () => {
    ...
    // console.log(39, this.state.allTransactions)
  };

  searchTransactions = async (s) => {
    console.log(43,s, typeof(s))
    if(s.length==0){console.log(44,true)}
    var txt = s.split();
        ...
        <View style={{ flexDirection: 'row' }}>
          ...
          <View
            ...
            <View style={{ flexDirection: 'row' }}>
              ...
              <Text style={styles.svT2}> {'' + item.DOT.toDate()}</Text>
            </View>
          </View>
        </View>
      </View>
  ...
  fetchMoreTransactions = async () => {
    ...
    if (this.state.searchTxt.length==0) {
      ...
      var txt = this.state.searchTxt.split();
      txt = txt[0].toUpperCase();
      var st = '';
      if (txt[0] === 'B') {
        st = 'bookId';
          ...
          {this.state.allTransactions.map((t) => {
            ...
            return (
              <View style={[styles.svV, {backgroundColor:typeColor}]}>
                <View style={{ flexDirection: 'row' }}>
                  <Text style={styles.svT1}>Book-ID:</Text>
                  <Text style={styles.svT2}> {t.bookId}</Text>
                </View>
                <View style={{ flexDirection: 'row' }}>
                  <Text style={styles.svT1}>Student-ID:</Text>
                  <Text style={styles.svT2}> {t.studentID}</Text>
                </View>
                <View style={{ flexDirection: 'row' }}>
                  <Text style={styles.svT1}>Type:</Text>
                  <Text style={styles.svT2}> {t.type}</Text>
                </View>
                <View style={{ flexDirection: 'row' }}>
                  <Text style={styles.svT1}>Date:</Text>
                  <Text style={styles.svT2}> {""+t.DOT.toDate()}</Text>
                </View>              
              </View>
            );
          })}
